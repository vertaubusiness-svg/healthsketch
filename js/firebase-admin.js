(async () => {
  ...
  let getFirestore, collection, addDoc, updateDoc, deleteDoc;
  ...
  try {
    ...
    if (loginScreen) {
      loginScreen.insertAdjacentHTML('beforeend',
        '<p style="color:#dc3545;text-align:center;margin-top:16px;font-size:13px;">' +
        'Firebase SDK 로드 실패.<br>네트워크 연결을 확인하거나 콘솔(F12)을 확인하세요.</p>');
    }
    return;
  }
  ...
  /* DOM 요소 존재 여부 확인 */
  const missing = ['login-form','login-email','login-password','login-btn','product-form']
  ...
  productForm.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      console.error('[firebase-admin] 저장 오류:', err);
  ...
  function loadProducts() {
    ...
    onSnapshot(q, (snapshot) => {
      ...
      console.error('[firebase-admin] Firestore 목록 오류:', err);
  ...
  async function confirmDelete(id, data) {
    ...
    try {
      ...
      console.error('[firebase-admin] 삭제 오류:', err);
